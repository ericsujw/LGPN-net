...
class SPADEResnetBlock(nn.Module):
    ...
    def forward(self, x, seg, seg_p, style_codes, obj_dic=None):
        ...
        ###########  Modifications 1
        dx = self.ace_0(x, seg, seg_p, style_codes, obj_dic)

        dx = self.conv_0(self.actvn(dx))

        dx = self.ace_1(dx, seg, seg_p, style_codes, obj_dic)
...
class Zencoder(torch.nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32, n_downsampling=2, norm_layer=nn.InstanceNorm2d, partial=1):
        ...
        #     mult = 2**i
        #     model += [PartialConv2d(ngf * mult, ngf * mult * 2, 3, 2, 1, multi_channel=True, bias=True),
