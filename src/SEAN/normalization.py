...
class ACE(nn.Module):
    ...
    def forward(self, x, segmap, segmap_plane, style_codes=None, obj_dic=None):
        ...
        segmap_plane = F.interpolate(segmap_plane, size=x.size()[2:], mode='nearest')
        ...
        if self.use_rgb:
            ...
            if self.status == 'UI_mode':
                ...
                for i in range(1):
                    for j in range(segmap_plane.shape[1]):

                        component_mask_area = torch.sum(segmap_plane.bool()[i, j])
                        ...
                        if component_mask_area > 0:
                            if obj_dic is None:
                                ...
                                if j < 2:
                                    ...
                                    middle_mu = F.relu(self.__getattr__('fc_mu3')(style_code_tmp))
                                component_mu = middle_mu.reshape(self.style_length, 1).expand(self.style_length,component_mask_area)

                                middle_avg[i].masked_scatter_(segmap_plane.bool()[i, j], component_mu)
                ...
                for i in range(b_size):
                    for j in range(segmap_plane.shape[1]):
                        ...
                        if component_mask_area > 0:
                            ...
                            if j > 2:
                                middle_mu = F.relu(self.__getattr__('fc_mu3')(style_codes[i][j]))
                            component_mu = middle_mu.reshape(self.style_length, 1).expand(self.style_length, component_mask_area)
